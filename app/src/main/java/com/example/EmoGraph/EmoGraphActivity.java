...
public class EmoGraphActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 저장된 감정 기록 가져오기
        String emotionRecords = sharedPreferences.getString("records", "No emotion data found.");
        if (!emotionRecords.equals("No emotion data found.")) {
            // 감정 기록을 파싱해서 리스트에 저장 (단순히 줄바꿈으로 구분)
            String[] recordsArray = emotionRecords.split("\n\n");
            for (String record : recordsArray) {
                emotionData.add(record);
            }
        }
        ...
        emotionTextView.setText(emotionRecords);
